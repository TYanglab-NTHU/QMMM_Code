...
CCDC_ident.sort()
print("Remove repeat entries from list")
before = len(CCDC_ident)
print("before :", before)
multiples = []
deldouble = []
for ent in CCDC_ident:
    if len(ent) > 6:
        ccdc_base = ent[0:6]
        printf("%8s  %s  ", ent, ccdc_base)
        if ccdc_base not in multiples:
            print("keep")
            multiples.append(ccdc_base)
        else:
            print("remove")
            deldouble.append(ent)
for ent in deldouble:
    CCDC_ident.remove(ent)
print("after  :", len(CCDC_ident))
print("removed:", before - len(CCDC_ident))
print()
