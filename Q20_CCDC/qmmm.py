################################################################################
# 27.11.2024 Q20_CCDC
# - write_output_file modified to get alligned numbers for 100.0% and 0.0%
# - add functions to set the H atom switch
# - add the switch to the function to read the input file
################################################################################

# set version string
Version='qm20-20241127'
# set directory
...
import time                     # used for waiting after writing the new file
...
# sort by distance
def my_sort_by_dist(ml):
  def my_key(ma):
    return(atom_dist(ma, gb.dnts['cent'][0]))
  ml.sort(key=my_key)
  return(ml)
...
def set_conju_angle(angle):
  if angle>0:
    angle = angle * 1.0
  if gb.VerboseFlag>0:
    print("Set the cut-off angle for twisted conjugated links")
    if angle<0:
      print("Check for twisted conjugated links turned off")
    if angle==0:
      print("Use the default (30.0 deg)")
    if angle>0.0:
      print(f'Set cut-off twist angle to %.1f degrees' % (angle))
  if angle==0.0:
    angle=30.0
  gb.conju_twist_angle = angle
  return()

def get_conju_angle():
  return(gb.conju_twist_angle)

def twisted(dihedral, threshold):
  if threshold<0:
    return(bool(False)) # assumed to be planar by default
  ang = abs(dihedral)
  ...
  if gb.mol.bonds[ind1].bond_type == 2 and gb.mol.bonds[ind2].bond_type == 1:
    if len(at3.neighbours)>0:
      ...
      if gb.VerboseFlag==2:
        ...
        printf("  %f", dihedral)
        if twisted(dihedral, gb.conju_twist_angle): printf("  twisted")
        else: printf("  conjugated")
    if gb.VerboseFlag==2: printf("\n")
    if twisted(dihedral, gb.conju_twist_angle): return(0)
    else: return(-2)
...
#        aux_file     name of the mol2 file with additional H atoms            #
################################################################################

def set_Hswitch(min_num):
  if gb.VerboseFlag>0:
    print("Set switch for missing H atoms")
  if min_num == -2:
    if gb.VerboseFlag>0:
      print('Hswitch = -2: set the default value of 10')
    gb.inp_min_H_atoms = 10
    return()
  if min_num == -1:
    if gb.VerboseFlag>0:
      print('Hswitch = -1: always look for missing H atoms')
    gb.inp_min_H_atoms = -1
    return()
  if min_num >= 0:
    if gb.VerboseFlag>0:
      printf('Hswitch = %i: add H atoms if the mol2 file contains %i or less H atoms\n', min_num, min_num)
    gb.inp_min_H_atoms = min_num
    return()
  print('Invalid switch for missing H atoms')
  exit()

def get_Hswitch():
  return(gb.inp_min_H_atoms)
...
def read_input_file(inp_name):
  ...
  if input_type==".mol2":
    ...
    if gb.inp_min_H_atoms == -1:
      org_inp_atom_num = len(gb.mol.atoms)
      if gb.VerboseFlag>0:
        print('  Mandatory test for missing H atoms')
        print(f'  %i atoms in %s' % (org_inp_atom_num, inp_name))
      gb.mol.add_hydrogens(mode='missing', add_sites=True)
      new_inp_atom_num = len(gb.mol.atoms)
      new_H_atoms = new_inp_atom_num - org_inp_atom_num
      if gb.VerboseFlag>0:
        print(f'  %i atoms after adding missing H atoms' % (new_inp_atom_num))
        print(f'  %i H atoms added' % (new_H_atoms))
        if new_H_atoms > 0:
          print(f'  write %s' % (aux_file))
      if new_H_atoms > 0:
      ...
      for at in gb.mol.atoms:
        if at.atomic_number==1:
          Hcnt += 1
      if Hcnt <= gb.inp_min_H_atoms:
        if gb.VerboseFlag>0:
          print(f'  %i H atoms in %s (<=%i)' % (Hcnt, inp_name, gb.inp_min_H_atoms))
...
# global VerboseFlag  controll the output to stdout                            #
# input  out_name     name of the output file with the path to it              #
# output stats        basic stats of the QM/MM separation                      #
################################################################################
...
def write_output_file(out_name):
  ...
  if gb.VerboseFlag > 0:
    ...
    printf("  All atoms     %4i\n", tat)
    printf("  QM atoms      %4i (%5.1f%%)\n", qat, 100.0*qat/tat)
    printf("  MM atoms      %4i (%5.1f%%)\n", mat, 100.0*mat/tat)
    printf("  All electrons %4i (based on Znuc)\n", tel)
    printf("  QM electrons  %4i (%5.1f%%)\n", qel, 100.0*qel/tel)
    printf("  MM electrons  %4i (%5.1f%%)\n", mel, 100.0*mel/tel)
    printf("  Links         %4i\n", len(link_list))
...
def set_metal_coor(coor_num):
  if gb.VerboseFlag>0:
    print("Set the minimum coordination number for metal atoms")
    if coor_num<0:
      print("Serach for distant neighbors turned off")
    if coor_num==0:
      print("Use the default (6, octahedral coordniation)")
    if coor_num>0:
      print(f'Set minimum coordination for metal atoms to %i' % (coor_num))
  if coor_num==0:
    coor_num=6
  gb.met_min_lig_num = coor_num
  return()

def get_metal_coor():
  return(gb.met_min_lig_num)

def toggle_H_removal():
  gb.del_H_from_list = not gb.del_H_from_list
  return(gb.del_H_from_list)
...
def distant_neighbors(max, vdw_fac):
  ...
  if len(gb.dnts['cent']) != 1:
    print('Distant neighbor search currently for cluster with 1 metal center ONLY')
    print('Try turning the distant neighbor search off')
    print('Use: qms.set_metal_coor(-1)')
    exit()
    ...
    for a2 in gb.mol.atoms: # loop over all atoms in the molecule
      ...
      if dist < max and a2 not in gb.qm:
        ...
        if dist<=vdw_sum:
          ...
          if in_mol_bonds(a1, a2)==-1 and bool(False):
            gb.mol.add_bond(1, a1, a2)
            bnd_cnt += 1
  new_atoms = my_clean_list(new_atoms)
  need = gb.met_min_lig_num - len(gb.dnts['close'])
  if gb.VerboseFlag > 0:
    print('  Minimum coordination:', gb.met_min_lig_num)
    print('  Direct neighbors    ', end=': ')
    print(len(gb.dnts['close']))
    print('  Ligands needed      ', end=': ')
    print(need)
    print('  Candidate list      :', len(new_atoms))
  if len(new_atoms) < need:
    print('  Not enough neighbors to meet minimum neighbors requirement')
    exit()
  ml=list(set(new_atoms))
  ml=my_sort_by_dist(ml)
  if gb.VerboseFlag>0:
    print('  Candidate list (with H) sorted by distance')
    for ma in ml:
      printf("  %-6s", my_label(ma))
      printf("  %6.4f\n", atom_dist(ma, gb.dnts['cent'][0]))
  # remove hydrogen atoms
  if gb.del_H_from_list:
    Hcnt = 0
    for i in range(len(ml)-1, -1, -1):
      if ml[i].atomic_number == 1:
        ml.remove(ml[i])
        Hcnt += 1
    if gb.VerboseFlag>0:
      print('  H atoms removed     :', Hcnt)
  else:
    print('  H atoms removed     : off')
  if len(ml) < need:
    print('  Not enough non H neighbors to meet minimum neighbors requirement')
    exit()
  if gb.VerboseFlag>0:
    print('  Used candidate list sorted by distance')
    for ma in ml:
      printf("  %-6s", my_label(ma))
      printf("  %6.4f\n", atom_dist(ma, gb.dnts['cent'][0]))
  ...
  for ne in itl:
    if gb.VerboseFlag==2:
      ...
      if len(ne.rings)>0:
        ...
        printf("  -")
      if ne.is_spiro  and len(ne.rings)>1:
        printf("  %i\n", len(ne.rings[1]))
      else:
        printf("  -\n")
  ...
  for qa in gb.qm:
    for na in qa.neighbours:
      ...
      if btype == "Single":
        ...
        if lp>0:
          if dihedral != 999:
            if twisted(dihedral, gb.conju_twist_angle):
              continue
            else:
              new_atoms.append(na)
          else:
...
def std_qmmm_sep():
  ...
  if len(gb.dnts['close']) < gb.met_min_lig_num:
    if gb.VerboseFlag>0:
      printf("{len(dnts['close'])<"+str(gb.met_min_lig_num)+"}, do distant neightbour search\n")
    gb.dnts['dist'] = distant_neighbors(3.0, 0.8)
  else:
    if gb.VerboseFlag>0:
      if gb.met_min_lig_num < 0:
        print(f'Distant neighbors search turned of (met_min_lig_num=%i)' % (gb.met_min_lig_num))
      else:
        printf("{len(dnts['close'])>="+str(gb.met_min_lig_num)+"}, skip distant neightbour search\n")
    gb.dnts['dist'] = []
  gb.dnts.update()
